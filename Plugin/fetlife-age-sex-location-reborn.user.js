...
FL_UI.Dialog.inject = function(id, title, html_content) {
  ...
  dialog.setAttribute(
    "style",
    "display: none; position: absolute; overflow: hidden; z-index: 1000; outline: 0px none;"
  );
  dialog.setAttribute(
    "class",
  ...
  html_string +=
    '<span class="ui-dialog-title" id="ui-dialog-title-' +
    id +
  ...
  html_string +=
    ...
    title +
    '" data-modal-height="280" data-modal-auto-open="false" class="modal ui-dialog-content ui-widget-content" id="' +
    id +
    '">';
...
FL_ASL.CONFIG = {
  ...
  api_url:
    "https://script.google.com/macros/s/AKfycbzUFY6t94AxfmbLtBpTUvkjUkAma_j-17rJPPURrkvG6ZR5SPc/exec?embedded=true",
  api_url_dev:
    "https://script.google.com/macros/s/AKfycbwxXfsfwPrEPLeXfVqFDNF0gX9wu0Iwj11Pli3xZYw/dev?embedded=true"
};
...
FL_ASL.init = function() {
  FL_ASL.CONFIG.search_form = document.querySelector(
    'form[action="/search"]'
  ).parentNode;
...
FL_ASL.createTabList = function() {
  ...
  html_string =
    '<li data-fl-asl-section-id="fetlife_asl_search_about"><a href="#">About FetLife ASL Search ' +
    GM_info.script.version +
    "</a></li>";
  html_string +=
    '<li class="in_section" data-fl-asl-section-id="fetlife_asl_search_extended"><a href="#">Extended A/S/L search</a></li>';
...
FL_ASL.attachSearchForm = function() {
  ...
  var url = FL_ASL.CONFIG.debug
    ? FL_ASL.CONFIG.api_url_dev
    : FL_ASL.CONFIG.api_url;
  ...
  html_string =
    '<p>The FetLife Age/Sex/Location Search user script allows you to search for profiles on <a href="https://fetlife.com/">FetLife</a> by age, sex, location, or orientation. This user script implements what is, as of this writing, the <a href="https://fetlife.com/improvements/78">most popular suggestion in the FetLife suggestion box</a>:</p>';
  html_string +=
    "<blockquote><p>Search for people by Location/Sex/Orientation/Age</p><p>Increase the detail of the kinkster search by allowing us to narrow the definition of the search by the traditional fields.</p></blockquote>";
  html_string +=
    "<p>With the FetLife Age/Sex/Location Search user script installed, a few clicks will save hours of time. Now you can find profiles that match your specified criteria in a matter of seconds. The script even lets you send a message to the profiles you found right from the search results list.</p>";
  html_string +=
    '<p>Stay up to date with the <a href="https://github.com/meitar/fetlife-aslsearch/">latest FetLife ASL Search improvements</a>. New versions add new features and improve search performance.</p>';
  html_string +=
    '<hr><p>FetLife ASL Search is provided as free software, but sadly grocery stores do not offer free food. If you like this script, please consider <a href="https://www.paypal.com/cgi-bin/webscr?cmd=_donations&business=kjeld@schouten-lebbing.nl&amount=&item_name=FetLife%20ASL%20Search">making a donation</a> to support its continued development. &hearts; Thank you. :)</p><hr><Br><br>';
  container.appendChild(
    FL_ASL.createSearchTab("fetlife_asl_search_about", html_string)
  );
  ...
  html_string +=
    ...
    FL_ASL.CONFIG.gasapp_url.split("?")[0] +
    '" width="100%" height="500px"><h2><a href="' +
    url +
    '" target="_blank">Open Extended A/S/L Search</a></h2></iframe>';
  html_string +=
    '<h4><a href="' +
    url +
    '" target="_blank">Open Extended A/S/L Search in a seperate window</a></h4>';
  html_string += "</div><!-- #fetlife_asl_search_extended_wrapper -->";
  var newdiv = container.appendChild(
    FL_ASL.createSearchTab("fetlife_asl_search_extended", html_string)
  );
...
FL_ASL.ProfileScraper.getSexualOrientation = function() {
  var x = $x(
    '//table//th[starts-with(., "orientation")]/following-sibling::td'
  );
  var ret = "";
  if (x.length) {
...
FL_ASL.ProfileScraper.getLookingFor = function() {
  var x = $x(
    '//table//th[starts-with(., "is looking for")]/following-sibling::td'
  );
  var ret = [];
  if (x.length) {
...
FL_ASL.ProfileScraper.getRelationships = function() {
  var x = $x(
    '//table//th[starts-with(., "relationship status")]/following-sibling::td//a'
  );
  var ret = [];
...
FL_ASL.ProfileScraper.getDsRelationships = function() {
  var x = $x(
    '//table//th[starts-with(., "D/s relationship status")]/following-sibling::td//a'
  );
  var ret = [];
  for (var i = 0; i < x.length; i++) {
    ret.push(x[i].href.match(/\d+$/)[0]);
...
FL_ASL.ProfileScraper.getBio = function() {
  var html = "";
  jQuery($x('//h3[@class][starts-with(., "About me")]'))
    .nextUntil("h3.bottom")
    .each(function() {
...
FL_ASL.ProfileScraper.getWebsites = function() {
  var x = $x(
    '//h3[@class="bottom"][starts-with(., "Websites")]/following-sibling::ul[1]//a'
  );
  var ret = [];
  for (var i = 0; i < x.length; i++) {
...
FL_ASL.ProfileScraper.getFetishesInto = function() {
  var x = $x(
    '//h3[@class="bottom"][starts-with(., "Fetishes")]/following-sibling::p[1]//a'
  );
  var ret = [];
  for (var i = 0; i < x.length; i++) {
    ret.push(x[i].textContent.trim());
...
FL_ASL.ProfileScraper.getFetishesCuriousAbout = function() {
  var x = $x(
    '//h3[@class="bottom"][starts-with(., "Fetishes")]/following-sibling::p[2]//a'
  );
  var ret = [];
  for (var i = 0; i < x.length; i++) {
    ret.push(x[i].textContent.trim());
...
FL_ASL.scrapeUserInList = function(node) {
  ...
  var loc_parts = jQuery(node)
    .find(".fl-member-card__location")
    .first()
    .text()
    .split(", ");
  var locality = "";
  ...
  var profile_data = {
    ...
    avatar_url: jQuery(node)
      .find("img")
      .first()
      .attr("src")
  };
  var member_info = jQuery(node)
    .find(".fl-member-card__info")
    .text()
    .trim();
...
FL_ASL.scrapeUserInOtherList = function(node) {
  var loc_parts = jQuery(node)
    .find("div.f6.lh-copy.fw4.silver.nowrap.truncate")
    .first()
    .text()
    .split(", ");
  var locality = "";
  ...
  var profile_data = {
    user_id: jQuery(node)
      ...
      .match(/\d+$/)[0],
    nickname: jQuery(node)
      .find("a.link.span.f5.fw7.secondary")
      .text()
      .trim(),
    location_locality: locality.trim(),
    location_region: region.trim(),
